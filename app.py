...
# ==================== رقم منصة SOKO LINK ====================
SOKO_PHONE = "0767063120"  # ضع رقم Mobile money الخاص بك هنا
...
translations = {
    ...
    "sw": {
        ...
        "payment_code": "Nambari ya malipo",
        "send_with_code": "Tuma malipo kwa nambari hii",
        "reference": "Kumbukumbu",
        "i_have_paid": "✅ Nimalipo",
            ...
            for p in filtered:
                ...
                if not is_unlocked:
                    ...
                    if st.button(f"🔓 {t('unlock_farmer')} (1,000 UGX)", key=f"unlock_{p['id']}"):
                        ...
                        st.session_state.pending_payments.append(payment_request)
                        
                        st.info(f"""
                        **📞 {t('payment_code')}**
                        -------------------
                        1. Open M-Pesa / Airtel Money
                        2. Send **1,000 UGX** to: **{SOKO_PHONE}**
                        3. In **Reference** field, enter: **{payment_code}**
                        4. Click **"{t('i_have_paid')}"** below
                        
                        🔑 **Code:** `{payment_code}`
                        """)
                        
                        if st.button(f"{t('i_have_paid')}", key=f"confirm_unlock_{p['id']}"):
                            st.session_state.unlocked_products.append(p['id'])
                            st.success(f"✅ {t('confirmed')}! {t('unlock_farmer')}")
                            st.rerun()
                else:
                    st.markdown(f"""
                ...
                if o["status"] == "pending" and not is_unlocked:
                    ...
                    if st.button(f"🔓 {t('unlock_buyer')} (1,000 UGX)", key=f"unlock_order_{o['id']}"):
                        ...
                        **📞 {t('payment_code')}**
                        ...
                        if st.button(f"{t('i_have_paid')}", key=f"confirm_unlock_order_{o['id']}"):
                            st.session_state.unlocked_orders.append(o['id'])
                            st.success(f"✅ {t('confirmed')}! {t('unlock_buyer')}")
                            st.rerun()
                else:
                    st.markdown(f"""
                    <div class='product-card'>
...
# ==================== لوحة التحكم (للمشرف) ====================
with st.expander(f"🔧 {t('admin_panel')}", expanded=False):
    st.subheader(t('pending_payments'))
    
    if st.session_state.pending_payments:
        for payment in st.session_state.pending_payments:
            if payment.get("status") == "pending":
                st.markdown(f"""
                **{payment.get('user', 'N/A')}** ({payment.get('phone', 'N/A')})  
                📦 {payment.get('product_name', 'N/A')}  
                💰 {payment.get('amount', 0):,} UGX  
                🔑 رمز: `{payment.get('code', 'N/A')}`  
                🕒 {payment.get('time', 'N/A')}
                """)
                
                if st.button(f"✅ {t('confirm_payment')}", key=f"admin_confirm_{payment.get('id')}"):
                    payment["status"] = "confirmed"
                    st.success(f"✅ تم تأكيد الدفع لـ {payment.get('user')}")
                    st.rerun()
    else:
        st.info("لا توجد دفعات منتظرة")
...
<div class='footer'>
    ...
    💳 M-Pesa / Airtel Money: {SOKO_PHONE}<br>
